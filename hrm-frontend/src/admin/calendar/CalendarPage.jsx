...
import {
  ...
  Calendar as CalendarIcon, List, Grid, Clock, MapPin,
  AlignLeft
} from "lucide-react";
...
const CalendarPage = () => {
  ...
  const getRangeForView = useCallback(() => {
    ...
    if (viewMode === "week") {
      ...
      return { start: toLocalDateStr(days[0]), end: toLocalDateStr(days[6]) };
    }
  ...
  const handleDeleteEvent = async (eventId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `https://hrm-backend-vvqg.onrender.com/api/calendar/events/${eventId}`,
        { headers: { "x-auth-token": token } }
      );
      fetchEvents();
      setShowEventDetails(false);
      ...
      <div style={{
        ...
        <div className="cal-topbar" style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px", flex: 1 }}>
            ...
            <h1 style={{ fontSize: "1rem", fontWeight: "600", color: t.textPrimary, margin: 0 }}>Calendar</h1>
          </div>
